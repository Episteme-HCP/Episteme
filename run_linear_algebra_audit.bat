...
REM Specialized Audit for Linear Algebra (Performance + Correctness)
...
call mvn clean test -pl episteme-benchmarks -am -Dtest=LinearAlgebraPerformanceAudit -Dorg.episteme.test.precision=normal -Dsurefire.failIfNoSpecifiedTests=false

echo [2/3] Running Linear Algebra Double Compliance Test...
call mvn test -pl episteme-benchmarks -Dtest=LinearAlgebraComplianceTest -Dsurefire.failIfNoSpecifiedTests=false

echo [3/3] Running Linear Algebra Float Compliance Test...
call mvn test -pl episteme-benchmarks -Dtest=LinearAlgebraFloatComplianceTest -Dsurefire.failIfNoSpecifiedTests=false
