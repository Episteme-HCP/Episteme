...
public class DistributedNBodyProvider implements NBodyProvider {
    ...
    public void computeForces(double[] positions, double[] masses, double[] forces, double G, double softening) {
        ...
        int rank = 0;
        if (ctx instanceof org.episteme.core.distributed.MPIDistributedContext) {
            rank = ((org.episteme.core.distributed.MPIDistributedContext) ctx).getRank();
        } 
        ...
        try {
            ...
            // If local context, we manually copy?
             if (ctx instanceof org.episteme.core.distributed.LocalDistributedContext) {
    ...
    public void step(double[] positions, double[] velocities, double[] masses, int numBodies, double G, double dt, double softening) {
        ...
        // Integration (usually only Rank 0 or distributed if we want)
        // For simplicity, let's assume each node integrates its own slice if we have full state.
        // But computeForces already syncs 'forces'.
    ...
    public void stepReal(Real[] positions, Real[] velocities, Real[] masses, int numBodies, Real G, Real dt, Real softening) {
        throw new UnsupportedOperationException("Distributed stepReal not implemented.");
    }
