...
package org.episteme.natural.physics.classical.waves.electromagnetism.providers;
...
import org.episteme.natural.physics.classical.waves.electromagnetism.MaxwellSource;
...
public class MulticoreMaxwellProvider implements MaxwellProvider {
    ...
    public Real[][] computeTensor(Vector4D point) {
        ...
        Real z = Real.of(point.getZ());

        Real[] totalE = new Real[] {Real.ZERO, Real.ZERO, Real.ZERO};
        Real[] totalB = new Real[] {Real.ZERO, Real.ZERO, Real.ZERO};

        for (MaxwellSource source : sources) {
            Real[] EB = source.computeFieldsReal(t, x, y, z);
            totalE[0] = totalE[0].add(EB[0]); totalE[1] = totalE[1].add(EB[1]); totalE[2] = totalE[2].add(EB[2]);
            totalB[0] = totalB[0].add(EB[3]); totalB[1] = totalB[1].add(EB[4]); totalB[2] = totalB[2].add(EB[5]);
        }
