...
public class SystematicTensorBenchmark implements SystematicBenchmark<TensorProvider> {
    ...
    private volatile Real resultSink;
    ...
    public String getDescription() {
        return "Tensor Hadamard Product (" + DIM + "x" + DIM + "x" + DIM + ")";
    }
    ...
    public void run() {
        if (tensorA != null && tensorB != null) {
            // Force computation and result usage to prevent JIT optimization
            resultSink = tensorA.multiply(tensorB).sum();
        }
    }
