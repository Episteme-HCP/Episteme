...
public class MainController {
    ...
    @FXML private TreeTableColumn<BenchmarkItem, String> modeColumn;
    ...
    private void updateUI(Locale locale) {
        try {
            ...
            modeColumn.setText(resources.getString("col.mode"));
    ...
    private void setupTable() {
        ...
        modeColumn.setCellValueFactory(param -> param.getValue().getValue().modeProperty());
    ...
    private java.util.concurrent.ExecutorService benchmarkExecutor = createExecutor();
    ...
    private java.util.concurrent.Future<?> currentSuiteFuture;
    ...
    private void runBenchmarkSuite(List<BenchmarkItem> items) {
        ...
        if (currentSuiteFuture != null && !currentSuiteFuture.isDone()) {
            // Already running, items added to thread-safe queue will be picked up
            return;
        }

        currentSuiteFuture = benchmarkExecutor.submit(() -> {
            try {
                ...
                while (!benchmarkQueue.isEmpty()) {
                    ...
                    completedCount++;
                    double progress = (double) completedCount / selectedCount;
                    Platform.runLater(() -> {
                ...
                Platform.runLater(() -> {
                    ...
                    statusBarLabel.setText("Suite Failed: " + e.getMessage());
                    updateCategoryStatuses(benchmarkTreeTable.getRoot());
                });
                e.printStackTrace();
            }
    ...
    private void handleStop() {
        if (currentSuiteFuture != null) {
            currentSuiteFuture.cancel(true);
        }
        
        // Force stop of any running task in the executor
        ...
        // Re-initialize executor for next runs
    ...
    private void reinitializeExecutor() {
        benchmarkExecutor = createExecutor();
    }
    ...
    private void executeSingle(BenchmarkItem item) {
        synchronized (benchmarkQueue) {
            ...
            if (!isProcessingQueue) {
                ...
                benchmarkExecutor.submit(this::processQueue);
            }
        }
    }
...
// ... (rest of runBenchmarkItem)
                    ...
                    // Lifecycle: setup -> warmup -> measure -> teardown
                    b.setup();
                    ...
                    try {
                        ...
                        Platform.runLater(() -> {
                            ...
                            String unit = (resources != null && resources.containsKey("unit.ops_sec")) ? resources.getString("unit.ops_sec") : "ops/s";
                            String formattedResult = String.format(java.util.Locale.US, "%.2f %s", throughput, unit);
                            item.resultProperty().set(formattedResult);
                            item.setScore(throughput);
                            updateChart(item, throughput);
                            addToHistory(item, formattedResult);
                        });
                    } finally {
                        b.teardown();
                    }
    ...
    private void updateChart(BenchmarkItem item, double value) {
        // Group by operation category AND mode
        String mode = item.getMode();
        String domain = item.getDomain();
        if (domain == null || domain.isBlank()) domain = item.getName();
        if (domain == null || domain.isBlank()) domain = "General";
        
        final String category = domain + (mode != null && !mode.isEmpty() ? " - " + mode : "");

        // X-axis bar label = provider
        String barLabel = item.getProvider();
        if (item.getLibrary() != null && !item.getLibrary().isBlank() && !item.getLibrary().equals(item.getProvider())) {
            barLabel += " (" + item.getLibrary() + ")";
        }
        ...
        // Update existing bar if already present for this provider in this category
        final String finalBarLabel = barLabel;
        for (XYChart.Data<String, Number> d : series.getData()) {
            if (d.getXValue().equals(finalBarLabel)) {
                d.setYValue(value);
                return;
            }
        }
    ...
    private BarChart<String, Number> getOrCreateOperationChart(String category) {
        ...
        String yLabel = (resources != null && resources.containsKey("lbl.ops_sec")) ? resources.getString("lbl.ops_sec") : "Ops/Sec";
