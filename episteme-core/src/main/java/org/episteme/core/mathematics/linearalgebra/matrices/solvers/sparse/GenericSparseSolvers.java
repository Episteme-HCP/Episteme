...
public class GenericSparseSolvers {
    ...
    public static <E> Vector<E> bicgstab(SparseLinearAlgebraProvider<E> provider, Matrix<E> A, Vector<E> b, Vector<E> x0, E tolerance, int maxIterations, Field<E> f) {
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            ...
            // Check for breakdown with epsilon clamping
            double breakdownThreshold = getThreshold(f);
            if (isSmaller(rho, breakdownThreshold, f)) {
                 // Try to recover by choosing a new r0? Or just warn and stop.
                 if (iter > 0) return x; // Return best guess
                 throw new ArithmeticException("BiCGSTAB breakdown: rho is " + rho + " (near zero). This usually indicates a singular or extremely ill-conditioned matrix.");
            }
            ...
            if (isSmaller(tDotT, breakdownThreshold, f)) {
                // Omega would be undefined, return best guess
                if (iter > 0) return x;
                throw new ArithmeticException("BiCGSTAB breakdown: omega denominator (t.t) is near zero at iteration " + iter);
            }
    ...
    private static <E> double getThreshold(Field<E> f) {
        ...
        if (zero instanceof Real r) {
            ...
            if (s.contains("RealFloat")) return 1e-8;
            if (s.contains("RealDouble")) return 1e-18;
            return 1e-40;
        }
        return 1e-45;
    }
    ...
    private static Class<?> componentType(Field<?> field) {
        ...
        if (zero instanceof org.episteme.core.mathematics.numbers.real.Real) return org.episteme.core.mathematics.numbers.real.Real.class;
        if (zero instanceof org.episteme.core.mathematics.numbers.complex.Complex) return org.episteme.core.mathematics.numbers.complex.Complex.class;
