...
public class SIMDRealFloatMatrix extends GenericMatrix<Real> implements AutoCloseable {
    ...
    public Matrix<Real> abs() { return applyOp(VectorOperators.ABS, (d) -> Math.abs(d)); }
    public Matrix<Real> sqrt() { return applyOp(VectorOperators.SQRT, (d) -> Math.sqrt(d)); }
    public Matrix<Real> log() { return applyOp(VectorOperators.LOG, (d) -> Math.log(d)); }
    public Matrix<Real> exp() { return applyOp(VectorOperators.EXP, (d) -> Math.exp(d)); }
    public Matrix<Real> sin() { return applyOp(VectorOperators.SIN, (d) -> Math.sin(d)); }
    public Matrix<Real> cos() { return applyOp(VectorOperators.COS, (d) -> Math.cos(d)); }
    public Matrix<Real> tan() { return applyOp(VectorOperators.TAN, (d) -> Math.tan(d)); }
    public Matrix<Real> asin() { return applyOp(VectorOperators.ASIN, (d) -> Math.asin(d)); }
    public Matrix<Real> acos() { return applyOp(VectorOperators.ACOS, (d) -> Math.acos(d)); }
    public Matrix<Real> atan() { return applyOp(VectorOperators.ATAN, (d) -> Math.atan(d)); }
    public Matrix<Real> sinh() { return applyOp(VectorOperators.SINH, (d) -> Math.sinh(d)); }
    public Matrix<Real> cosh() { return applyOp(VectorOperators.COSH, (d) -> Math.cosh(d)); }
    public Matrix<Real> tanh() { return applyOp(VectorOperators.TANH, (d) -> Math.tanh(d)); }
    public Matrix<Real> asinh() { return applyOp(null, (x) -> Math.log(x + Math.sqrt(x * x + 1.0))); }
    public Matrix<Real> acosh() { return applyOp(null, (x) -> Math.log(x + Math.sqrt(x * x - 1.0))); }
    public Matrix<Real> atanh() { return applyOp(null, (x) -> 0.5 * Math.log((1.0 + x) / (1.0 - x))); }
    public Matrix<Real> cbrt() { return applyOp(VectorOperators.CBRT, (d) -> Math.cbrt(d)); }
    public Matrix<Real> log10() { return applyOp(VectorOperators.LOG10, (d) -> Math.log10(d)); }

    public Matrix<Real> powScalar(float exponent) {
        float[] res = new float[data.length];
        int i = 0;
        final VectorSpecies<Float> species = SPECIES;
        FloatVector vExp = FloatVector.broadcast(species, exponent);
        ...
        for (; i < data.length; i++) res[i] = (float) Math.pow(data[i], exponent);
    ...
    private Matrix<Real> applyOp(jdk.incubator.vector.VectorOperators.Unary op, java.util.function.DoubleUnaryOperator scalarFallback) {
        ...
        for (; i < data.length; i++) res[i] = (float) scalarFallback.applyAsDouble(data[i]);
        return new SIMDRealFloatMatrix(storage.rows(), storage.cols(), res);
    }
