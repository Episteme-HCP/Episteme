...
if [ -f /etc/debian_version ]; then
    # Enable contrib, non-free and non-free-firmware for Debian
    ...
    echo "[WARN] Non-Debian system detected. Basic deps might need manual install."
fi
...
if ! sdk list java | grep -q "25-open"; then
    ...
    sdk install java 25-open || echo "[WARN] Failed to install JDK 25 via SDKMAN. If already installed, ignore."
else
...
if ! command -v nvidia-smi &> /dev/null; then
    ...
    if [ -f /etc/debian_version ]; then
        sudo -E apt-get update
        sudo -E apt-get install -y linux-headers-$(uname -r) build-essential -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold"
