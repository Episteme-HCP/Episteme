...
public class NativeMiniAudioBackend implements AudioBackend, NativeBackend, CPUBackend { 
    ...
    public void load(String path) throws Exception {
        ...
        // miniaudio's ma_decoder struct is very large (~4-8KB depending on version/platform).
        // Allocating a generous buffer to avoid buffer-overrun segfaults.
        decoder = sessionArena.allocate(8192);
        decoder.fill((byte) 0);
        MemorySegment pathSegment = NativeSafe.allocateFrom(sessionArena, path);
        ...
        try {
            ...
            // 2. Initialize Device
            // miniaudio's ma_device struct is also very large (~4-8KB).
            device = sessionArena.allocate(8192);
            device.fill((byte) 0);
            MemorySegment deviceConfig = sessionArena.allocate(MA_DEVICE_CONFIG_LAYOUT);
            deviceConfig.fill((byte) 0);
        ...
        logger.info("[NativeMiniAudio] Loaded: {}", path);
    }
