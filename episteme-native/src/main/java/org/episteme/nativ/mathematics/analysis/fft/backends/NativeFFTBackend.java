...
package org.episteme.nativ.mathematics.analysis.fft.backends;
...
public class NativeFFTBackend implements FFTProvider, CPUBackend, NativeBackend {
    ...
    public double[][] transform(double[] real, double[] imag) {
        ...
        try (Arena arena = Arena.ofConfined();
            ...
            MemorySegment in = NativeSafe.allocate(arena, ValueLayout.JAVA_DOUBLE, n);
            for (int i = 0; i < n; i++)
                in.setAtIndex(ValueLayout.JAVA_DOUBLE, i, real[i]);
            // Output for r2c is (n/2+1) complex numbers
            MemorySegment out = NativeSafe.allocate(arena, ValueLayout.JAVA_DOUBLE, (n / 2 + 1) * 2);
    ...
    public double[][] inverseTransform(double[] real, double[] imag) {
        ...
        try (Arena arena = Arena.ofConfined();
             ResourceTracker tracker = new ResourceTracker()) {
            MemorySegment in = NativeSafe.allocate(arena, ValueLayout.JAVA_DOUBLE, real.length * 2);
            for (int i = 0; i < real.length; i++) {
                in.setAtIndex(ValueLayout.JAVA_DOUBLE, i * 2, real[i]);
                in.setAtIndex(ValueLayout.JAVA_DOUBLE, i * 2 + 1, imag[i]);
            }
            MemorySegment out = NativeSafe.allocate(arena, ValueLayout.JAVA_DOUBLE, outN);
    ...
    public Complex[] transformComplex(Complex[] data) {
        ...
        try (Arena arena = Arena.ofConfined();
             ResourceTracker tracker = new ResourceTracker()) {
            MemorySegment in = NativeSafe.allocate(arena, ValueLayout.JAVA_DOUBLE, n * 2);
            for (int i = 0; i < n; i++) {
                in.setAtIndex(ValueLayout.JAVA_DOUBLE, i * 2, data[i].real());
                in.setAtIndex(ValueLayout.JAVA_DOUBLE, i * 2 + 1, data[i].imaginary());
            }
            MemorySegment out = NativeSafe.allocate(arena, ValueLayout.JAVA_DOUBLE, n * 2);
    ...
    public float[][] transform(float[] real, float[] imag) {
        ...
        try (Arena arena = Arena.ofConfined();
             ResourceTracker tracker = new ResourceTracker()) {
            MemorySegment in = NativeSafe.allocate(arena, ValueLayout.JAVA_FLOAT, n);
            for (int i = 0; i < n; i++)
                in.setAtIndex(ValueLayout.JAVA_FLOAT, i, real[i]);
            MemorySegment out = NativeSafe.allocate(arena, ValueLayout.JAVA_FLOAT, (n / 2 + 1) * 2);
    ...
    public float[][] inverseTransform(float[] real, float[] imag) {
        ...
        try (Arena arena = Arena.ofConfined();
             ResourceTracker tracker = new ResourceTracker()) {
            MemorySegment in = NativeSafe.allocate(arena, ValueLayout.JAVA_FLOAT, real.length * 2);
            for (int i = 0; i < real.length; i++) {
                in.setAtIndex(ValueLayout.JAVA_FLOAT, i * 2, real[i]);
                in.setAtIndex(ValueLayout.JAVA_FLOAT, i * 2 + 1, imag[i]);
            }
            MemorySegment out = NativeSafe.allocate(arena, ValueLayout.JAVA_FLOAT, outN);
    ...
    public double[][][] transform2D(double[][] real, double[][] imag) {
        ...
        try (Arena arena = Arena.ofConfined();
            ...
            MemorySegment in = NativeSafe.allocate(arena, ValueLayout.JAVA_DOUBLE, totalElements * 2);
            for(int i=0; i<n0; i++) {
                for(int j=0; j<n1; j++) {
            ...
            MemorySegment out = NativeSafe.allocate(arena, ValueLayout.JAVA_DOUBLE, totalElements * 2);
    ...
    public float[][][] transform2D(float[][] real, float[][] imag) {
        ...
        try (Arena arena = Arena.ofConfined();
             ResourceTracker tracker = new ResourceTracker()) {
            MemorySegment in = NativeSafe.allocate(arena, ValueLayout.JAVA_FLOAT, totalElements * 2);
            for(int i=0; i<n0; i++) {
                for(int j=0; j<n1; j++) {
                    long offset = ((long) i * n1 + j) * 2;
            ...
            MemorySegment out = NativeSafe.allocate(arena, ValueLayout.JAVA_FLOAT, totalElements * 2);
