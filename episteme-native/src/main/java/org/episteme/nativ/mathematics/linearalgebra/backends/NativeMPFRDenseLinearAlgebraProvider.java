...
public class NativeMPFRDenseLinearAlgebraProvider implements LinearAlgebraBackend<Real>, NativeBackend, CPUBackend {
            ...
            for (int i = 0; i < m; i++) {
                for (int j = 0; j < n; j++) {
                    if (isComplex) {
                        ...
                        int _ = (int) MPFR_SET_STR.invokeExact(sumR, arena.allocateFrom("0"), 10, rnd);
                        int _ = (int) MPFR_SET_STR.invokeExact(sumI, arena.allocateFrom("0"), 10, rnd);
                        ...
                        for (int l = 0; l < k; l++) {
                            ...
                            // ar*br - ai*bi
                            int _ = (int) MPFR_MUL.invokeExact(temp1, ar, br, rnd);
                            int _ = (int) MPFR_MUL.invokeExact(temp2, ai, bi, rnd);
                            int _ = (int) MPFR_SUB.invokeExact(temp1, temp1, temp2, rnd);
                            int _ = (int) MPFR_ADD.invokeExact(sumR, sumR, temp1, rnd);

                            // ar*bi + ai*br
                            int _ = (int) MPFR_MUL.invokeExact(temp1, ar, bi, rnd);
                            int _ = (int) MPFR_MUL.invokeExact(temp2, ai, br, rnd);
                            int _ = (int) MPFR_ADD.invokeExact(temp1, temp1, temp2, rnd);
                            int _ = (int) MPFR_ADD.invokeExact(sumI, sumI, temp1, rnd);
                        }
                    } else {
                        MemorySegment sum = getMPFR(h_C, i, j, n, 0, false);
                        int _ = (int) MPFR_SET_STR.invokeExact(sum, arena.allocateFrom("0"), 10, rnd);

                        for (int l = 0; l < k; l++) {
                            MemorySegment a1 = getMPFR(h_A, i, l, k, 0, false);
                            MemorySegment b1 = getMPFR(h_B, l, j, n, 0, false);
                            int _ = (int) MPFR_MUL.invokeExact(temp1, a1, b1, rnd);
                            int _ = (int) MPFR_ADD.invokeExact(sum, sum, temp1, rnd);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < m; i++) {
                if (isComplex) {
                    ...
                    for (int l = 0; l < k; l++) {
                        ...
                        // ar*xr - ai*xi
                        int _ = (int) MPFR_MUL.invokeExact(temp1, ar, xr, rnd);
                        int _ = (int) MPFR_MUL.invokeExact(temp2, ai, xi, rnd);
                        int _ = (int) MPFR_SUB.invokeExact(temp1, temp1, temp2, rnd);
                        int _ = (int) MPFR_ADD.invokeExact(sumR, sumR, temp1, rnd);
                        
                        // ar*xi + ai*xr
                        int _ = (int) MPFR_MUL.invokeExact(temp1, ar, xi, rnd);
                        int _ = (int) MPFR_MUL.invokeExact(temp2, ai, xr, rnd);
                        int _ = (int) MPFR_ADD.invokeExact(temp1, temp1, temp2, rnd);
                        int _ = (int) MPFR_ADD.invokeExact(sumI, sumI, temp1, rnd);
                    }
                } else {
                    ...
                    int _ = (int) MPFR_SET_STR.invokeExact(sum, arena.allocateFrom("0"), 10, rnd);
                    for (int l = 0; l < k; l++) {
                        int _ = (int) MPFR_MUL.invokeExact(temp1, getMPFR(h_A, i, l, k, 0, false), getMPFRVector(h_X, l, 0, false), rnd);
                        int _ = (int) MPFR_ADD.invokeExact(sum, sum, temp1, rnd);
                    }
                }
            }
    ...
    private MemorySegment initVector(Vector<Real> v, Arena arena, long prec, boolean isComplex) throws Throwable {
        ...
        for (int i = 0; i < dim; i++) {
            ...
            if (isComplex) {
                ...
                int _ = (int) MPFR_SET_STR.invokeExact(getMPFRVector(vec, i, 0, true), arena.allocateFrom(cv.getReal().bigDecimalValue().toPlainString()), 10, 0);
                int _ = (int) MPFR_SET_STR.invokeExact(getMPFRVector(vec, i, 1, true), arena.allocateFrom(cv.getImaginary().bigDecimalValue().toPlainString()), 10, 0);
            } else {
                Real rv = (Real) val;
                int _ = (int) MPFR_SET_STR.invokeExact(getMPFRVector(vec, i, 0, false), arena.allocateFrom(rv.bigDecimalValue().toPlainString()), 10, 0);
            }
        }
        return vec;
    }
    ...
    public Matrix<Real> scale(Real scalar, Matrix<Real> a) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            for (int i = 0; i < m * n * multiplier; i++) {
                MemorySegment ra = h_A.asSlice(i * MPFR_LAYOUT.byteSize(), MPFR_LAYOUT);
                MemorySegment rc = h_C.asSlice(i * MPFR_LAYOUT.byteSize(), MPFR_LAYOUT);
                int _ = (int) MPFR_MUL.invokeExact(rc, ra, s, rnd);
            }
            Matrix<Real> res = backToMatrix(h_C, m, n, arena, a.getScalarRing(), isComplex);
    ...
    private MemorySegment initMatrix(Matrix<Real> m, Arena arena, long prec, boolean isComplex) throws Throwable {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                ...
                if (isComplex) {
                    org.episteme.core.mathematics.numbers.complex.Complex cv = (org.episteme.core.mathematics.numbers.complex.Complex) val;
                    int _ = (int) MPFR_SET_STR.invokeExact(getMPFR(mat, i, j, cols, 0, true), arena.allocateFrom(cv.getReal().bigDecimalValue().toPlainString()), 10, 0);
                    int _ = (int) MPFR_SET_STR.invokeExact(getMPFR(mat, i, j, cols, 1, true), arena.allocateFrom(cv.getImaginary().bigDecimalValue().toPlainString()), 10, 0);
                } else {
                    Real rv = (Real) val;
                    int _ = (int) MPFR_SET_STR.invokeExact(getMPFR(mat, i, j, cols, 0, false), arena.allocateFrom(rv.bigDecimalValue().toPlainString()), 10, 0);
                }
            }
        }
        return mat;
    ...
    public Matrix<Real> transpose(Matrix<Real> a) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            for (int i = 0; i < m; i++) {
                for (int j = 0; j < n; j++) {
                    if (isComplex) {
                        int _ = (int) MPFR_SET.invokeExact(getMPFR(h_C, j, i, m, 0, true), getMPFR(h_A, i, j, n, 0, true), rnd);
                        int _ = (int) MPFR_SET.invokeExact(getMPFR(h_C, j, i, m, 1, true), getMPFR(h_A, i, j, n, 1, true), rnd);
                    } else {
                        int _ = (int) MPFR_SET.invokeExact(getMPFR(h_C, j, i, m, 0, false), getMPFR(h_A, i, j, n, 0, false), rnd);
                    }
                }
            }
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            for (int k = 0; k < n; k++) {
                ...
                for (int i = k + 1; i < n; i++) {
                    if (isComplex) {
                        ...
                        int _ = (int) MPFR_DIV.invokeExact(tempFactor, getMPFR(h_A, i, k, n, 0, false), getMPFR(h_A, k, k, n, 0, false), rnd);
            ...
            for (int i = n - 1; i >= 0; i--) {
                if (isComplex) {
                    ...
                    int _ = (int) MPFR_SUB.invokeExact(xi, getMPFRVector(h_B, i, 0, false), sum, rnd);
                    int _ = (int) MPFR_DIV.invokeExact(xi, xi, getMPFR(h_A, i, i, n, 0, false), rnd);
                }
            }
    ...
    public Matrix<Real> inverse(Matrix<Real> a) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            for (int i = 0; i < n; i++) {
                if (isComplex) {
                    int _ = (int) MPFR_SET_UI.invokeExact(getMPFR(h_Inv, i, i, n, 0, true), 1L, rnd);
                    int _ = (int) MPFR_SET_UI.invokeExact(getMPFR(h_Inv, i, i, n, 1, true), 0L, rnd);
                } else {
                    int _ = (int) MPFR_SET_UI.invokeExact(getMPFR(h_Inv, i, i, n, 0, false), 1L, rnd);
                }
            }
            ...
            for (int k = 0; k < n; k++) {
                ...
                if (isComplex) {
                    ...
                    int _ = (int) MPFR_SET.invokeExact(pR, getMPFR(h_A, k, k, n, 0, true), rnd);
                    int _ = (int) MPFR_SET.invokeExact(pI, getMPFR(h_A, k, k, n, 1, true), rnd);
                    ...
                    int _ = (int) MPFR_SET_UI.invokeExact(getMPFR(h_A, k, k, n, 0, true), 1L, rnd);
                    int _ = (int) MPFR_SET_UI.invokeExact(getMPFR(h_A, k, k, n, 1, true), 0L, rnd);
                } else {
                    MemorySegment pivotVal = arena.allocate(MPFR_LAYOUT);
                    MPFR_INIT2.invokeExact(pivotVal, prec);
                    int _ = (int) MPFR_SET.invokeExact(pivotVal, getMPFR(h_A, k, k, n, 0, false), rnd);
                    
                    for (int j = 0; j < n; j++) {
                        int _ = (int) MPFR_DIV.invokeExact(getMPFR(h_A, k, j, n, 0, false), getMPFR(h_A, k, j, n, 0, false), pivotVal, rnd);
                        int _ = (int) MPFR_DIV.invokeExact(getMPFR(h_Inv, k, j, n, 0, false), getMPFR(h_Inv, k, j, n, 0, false), pivotVal, rnd);
                    }
                    int _ = (int) MPFR_SET_UI.invokeExact(getMPFR(h_A, k, k, n, 0, false), 1L, rnd);
                }
