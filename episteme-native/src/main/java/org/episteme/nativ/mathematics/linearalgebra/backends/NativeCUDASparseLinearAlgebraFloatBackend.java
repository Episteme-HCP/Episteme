...
public class NativeCUDASparseLinearAlgebraFloatBackend<E extends FieldElement<E>> implements SparseLinearAlgebraProvider<E>, NativeBackend, GPUBackend {
    ...
    public Vector<E> multiply(Matrix<E> a, Vector<E> x) {
        ...
        try (ResourceTracker tracker = new ResourceTracker()) {
            ...
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_val, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, toFloatArray(sa)), (long)nnz * 4, CUDAManager.CUDA_MEMCPY_H_TO_D));
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_x, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, toFloatArray(x)), (long)k * 4, CUDAManager.CUDA_MEMCPY_H_TO_D));
    ...
    private Vector<E> multiplyComplex(Matrix<E> a, Vector<E> x) {
        ...
        try (ResourceTracker tracker = new ResourceTracker()) {
            ...
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_rowPtr, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_INT, sa.getRowPointers()), (long)(m + 1) * 4, CUDAManager.CUDA_MEMCPY_H_TO_D));
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_colIdx, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_INT, sa.getColIndices()), (long)nnz * 4, CUDAManager.CUDA_MEMCPY_H_TO_D));
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_val, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, toComplexFloatArray(sa)), (long)nnz * 8, CUDAManager.CUDA_MEMCPY_H_TO_D));
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_x, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, toComplexFloatVec(x)), (long)k * 8, CUDAManager.CUDA_MEMCPY_H_TO_D));
    ...
    private void spmv_internal(int m, int k, int nnz, MemorySegment d_rowPtr, MemorySegment d_colIdx, MemorySegment d_val, MemorySegment d_x, MemorySegment d_y, Arena arena, ResourceTracker tracker, int dataType) throws Throwable {
        ...
        MemorySegment alpha = NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, 1.0f);
        MemorySegment beta = NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, 0.0f);
        if (dataType == 6) { // CUDA_C_32F
            alpha = NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, 1.0f, 0.0f);
            beta = NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, 0.0f, 0.0f);
        }
    ...
    public Matrix<E> toDense(Matrix<E> a) {
        ...
        try (ResourceTracker tracker = new ResourceTracker()) {
            ...
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_rowPtr, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_INT, sa.getRowPointers()), (long)(m + 1) * 4, CUDAManager.CUDA_MEMCPY_H_TO_D));
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_colIdx, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_INT, sa.getColIndices()), (long)nnz * 4, CUDAManager.CUDA_MEMCPY_H_TO_D));
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_val, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, isComplex ? toComplexFloatArray(sa) : toFloatArray(sa)), (long)nnz * elemSize, CUDAManager.CUDA_MEMCPY_H_TO_D));
    ...
    public Matrix<E> fromDense(Matrix<E> a) {
        ...
        try (ResourceTracker tracker = new ResourceTracker()) {
            Arena arena = tracker.track(Arena.ofConfined(), Arena::close);
            MemorySegment d_dense = malloc((long)m * n * elemSize, tracker);
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_dense, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, isComplex ? toComplexFloatArrayGeneric(a) : toFloatArrayGeneric(a)), (long)m * n * elemSize, CUDAManager.CUDA_MEMCPY_H_TO_D));
    ...
    public Vector<E> conjugateGradient(Matrix<E> a, Vector<E> b, Vector<E> x0, E tolerance, int maxIterations) {
        ...
        try (ResourceTracker tracker = new ResourceTracker()) {
            ...
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_colIdx, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_INT, sa.getColIndices()), (long)nnz * 4, CUDAManager.CUDA_MEMCPY_H_TO_D));
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_val, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, isComplex ? toComplexFloatArray(sa) : toFloatArray(sa)), (long)nnz * elemSize, CUDAManager.CUDA_MEMCPY_H_TO_D));
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_b, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, isComplex ? toComplexFloatVec(b) : toFloatArray(b)), (long)n * elemSize, CUDAManager.CUDA_MEMCPY_H_TO_D));

            if (x0 != null) {
                checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_x, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, isComplex ? toComplexFloatVec(x0) : toFloatArray(x0)), (long)n * elemSize, CUDAManager.CUDA_MEMCPY_H_TO_D));
            } else {
    ...
    @Override public E dot(Vector<E> a, Vector<E> b) {
        ...
        try (ResourceTracker tracker = new ResourceTracker()) {
            ...
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_a, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, isComplex ? toComplexFloatVec(a) : toFloatArray(a)), (long)n * (isComplex ? 8 : 4), CUDAManager.CUDA_MEMCPY_H_TO_D));
            checkCuda((int) NativeSafe.invoke(CUDAManager.CUDA_MEMCPY, d_b, NativeSafe.allocateFrom(NativeSafe, arena, ValueLayout.JAVA_FLOAT, isComplex ? toComplexFloatVec(b) : toFloatArray(b)), (long)n * (isComplex ? 8 : 4), CUDAManager.CUDA_MEMCPY_H_TO_D));
