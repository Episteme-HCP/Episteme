...
package org.episteme.nativ.physics.classical.mechanics.collision.backends.genesis;
...
public class GenesisWorld implements PhysicsWorldBridge {
    ...
    public void addRigidBody(RigidBody body) {
        ...
        org.episteme.core.mathematics.linearalgebra.Vector<org.episteme.core.mathematics.numbers.real.Real> vel = body.getVelocity();
    ...
    public void removeRigidBody(RigidBody body) {
        int idx = -1;
        for (int i = 0; i < n; i++) {
            if (bodies.get(i).getEpistemeBody() == body) { idx = i; break; }
        }
        if (idx < 0) return;
        bodies.remove(idx);
        // Compact flat arrays
    ...
    /** Sync flat arrays back to Episteme RigidBody objects. */
    private void pushStates() {
        for (int i = 0; i < n; i++) {
            GenesisRigidBody gb = bodies.get(i);
            RigidBody body = gb.getEpistemeBody();
            body.setPosition(org.episteme.core.mathematics.linearalgebra.Vector.of(
                java.util.Arrays.asList(
                    org.episteme.core.mathematics.numbers.real.Real.of(px[i]),
                    org.episteme.core.mathematics.numbers.real.Real.of(py[i]),
                    org.episteme.core.mathematics.numbers.real.Real.of(pz[i])),
                org.episteme.core.mathematics.sets.Reals.getInstance()));
            body.setVelocity(org.episteme.core.mathematics.linearalgebra.Vector.of(
                java.util.Arrays.asList(
                    org.episteme.core.mathematics.numbers.real.Real.of(vx[i]),
                    org.episteme.core.mathematics.numbers.real.Real.of(vy[i]),
                    org.episteme.core.mathematics.numbers.real.Real.of(vz[i])),
                org.episteme.core.mathematics.sets.Reals.getInstance()));
        }
    }
