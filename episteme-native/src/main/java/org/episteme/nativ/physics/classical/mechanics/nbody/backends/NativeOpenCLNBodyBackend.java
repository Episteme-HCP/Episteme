...
public class NativeOpenCLNBodyBackend implements NBodyProvider, GPUBackend, NativeBackend {
            ...
            for (cl_platform_id platform : platforms) {
                ...
                if (numDevicesArray[0] > 0) {
                    ...
                    for (cl_device_id dev : devices) {
                        ...
                        clGetDeviceInfo(dev, CL_DEVICE_NAME, 0, null, sizeArray);
                        byte[] buffer = new byte[(int)sizeArray[0]];
                        clGetDeviceInfo(dev, CL_DEVICE_NAME, buffer.length, Pointer.to(buffer), null);
                        String deviceName = new String(buffer).trim();
                        deviceList.add(new GPUBackend.DeviceInfo(deviceName, 0, 0, "OpenCL Device"));
                    }
                }
            }
    ...
    public void copyToGPU(long gpuHandle, DoubleBuffer hostBuffer, long sizeBytes) {
        ...
        try {
            field = cl_mem.class.getDeclaredField("nativePointer");
            field.setAccessible(true);
            field.setLong(mem, gpuHandle);
        } catch (Exception e) {
            throw new RuntimeException("Failed to wrap native OpenCL handle", e);
        }
        clEnqueueWriteBuffer(ctx.getCommandQueue(), mem, CL_TRUE, 0, sizeBytes, Pointer.to(hostBuffer), 0, null, null);
    }
    ...
    public void copyFromGPU(long gpuHandle, DoubleBuffer hostBuffer, long sizeBytes) {
        ...
        clEnqueueReadBuffer(ctx.getCommandQueue(), mem, CL_TRUE, 0, sizeBytes, Pointer.to(hostBuffer), 0, null, null);
    }
    ...
    public void stepReal(Real[] positions, Real[] velocities, Real[] masses, int numBodies, Real G, Real dt, Real softening) {
        double[] dPos = new double[positions.length];
        double[] dVel = new double[velocities.length];
        double[] dMass = new double[masses.length];
        for (int i = 0; i < numBodies; i++) {
            dPos[i*3] = positions[i*3].doubleValue();
            dPos[i*3+1] = positions[i*3+1].doubleValue();
            dPos[i*3+2] = positions[i*3+2].doubleValue();
            dVel[i*3] = velocities[i*3].doubleValue();
            dVel[i*3+1] = velocities[i*3+1].doubleValue();
            dVel[i*3+2] = velocities[i*3+2].doubleValue();
            dMass[i] = masses[i].doubleValue();
        }
        step(dPos, dVel, dMass, numBodies, G.doubleValue(), dt.doubleValue(), softening.doubleValue());
        for (int i = 0; i < numBodies; i++) {
            positions[i*3] = Real.of(dPos[i*3]);
            positions[i*3+1] = Real.of(dPos[i*3+1]);
            positions[i*3+2] = Real.of(dPos[i*3+2]);
            velocities[i*3] = Real.of(dVel[i*3]);
            velocities[i*3+1] = Real.of(dVel[i*3+1]);
            velocities[i*3+2] = Real.of(dVel[i*3+2]);
        }
    }
